		...
		<div id="content">
			...
			<?php if ( $this->_['content']['posts'] ) : ?>
				...
				<?php
					...
					$month = $this->_['meta']['month'];
					$year = $this->_['meta']['year'];
					$max = ceil($this->_['content']['count']/10);
				
					if ( $page > 1 )
					{
						echo '<a class="prev_page" href="#!/archive/' . $month . '/' . $year . '/' . ($page-1) . '/">Vorherige Seite</a>';
					}
				
					if ( $max > $page )
					{
						echo '<a class="next_page" href="#!/archive/' . $month . '/' . $year . '/' . ($page+1) . '/">N&auml;chste Seite</a>';
					}
				?>
				</div>
			<?php endif; ?>
